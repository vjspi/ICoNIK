...
def forward_differences(x):    
    x_fwd = torch.zeros((x.ndim,) + x.shape, device=x.device)
...
def backward_differences(x):
    ...
    x_bwd = torch.zeros(x.shape, device=x.device)
...
class Nabla(torch.nn.Module):
    ...
    def tv(self, x):
        Dx = self.forward(x)
        return torch.sum(torch.sqrt(torch.sum(Dx.abs()**2, 0)))
...
class NablaTest(unittest.TestCase):
    ...
    def test_2d_tv(self):
        ...
        A = Nabla(mode='2d')
        tv_norm = A.tv(x)
        print('TV-NORM', tv_norm.detach().cpu().numpy())
